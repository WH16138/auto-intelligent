...
with st.expander("코드 구조 (필요할 때 펼치기)"):
    st.markdown(
        """
...
with st.expander("자주 묻는 질문 (간략)"):
    st.markdown(
        """
        - **데이터가 없을 때?** 01_upload에서 샘플 버튼을 눌러보세요.
        - **기본값만으로도 될까?** 각 단계는 안전한 기본 설정을 제공합니다. 먼저 실행 후, 필요 시 세부 조정하세요.
        - **긴 출력이 부담될 때?** 각 페이지에서 미리보기/expander를 활용해 요약만 확인할 수 있습니다.
        """
    )
